<?php $__env->startSection('title', 'TripWise — Performance Analytics'); ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(document.getElementById('perfTrendAnalyticsChart'), {
        type: 'line',
        data: {
            labels: <?php echo json_encode($perfTrend->pluck('month')->toArray()); ?>,
            datasets: [{
                label: 'Performance Score',
                data: <?php echo json_encode($perfTrend->pluck('avg_score')->toArray()); ?>,
                borderColor: '#F44336',
    ...
    new Chart(document.getElementById('kpiAnalyticsChart'), {
        type: 'bar',
        data: {
            labels: <?php echo json_encode($kpiByCategory->pluck('kpi_category')->toArray()); ?>,
            datasets: [{
                label: 'KPI Score',
                data: <?php echo json_encode($kpiByCategory->pluck('avg_achievement')->toArray()); ?>,
    ...
    new Chart(document.getElementById('safetyAnalyticsChart'), {
        type: 'pie',
        data: {
            labels: <?php echo json_encode(array_keys($safetyDistribution)); ?>,
            datasets: [{ data: <?php echo json_encode(array_values($safetyDistribution)); ?>, backgroundColor: ['#10b981', '#3b82f6', '#f59e0b', '#ef4444'] }]
        },
    ...
    new Chart(document.getElementById('attendanceAnalyticsChart'), {
        type: 'line',
        data: {
            labels: <?php echo json_encode($attendanceTrend->pluck('month')->toArray()); ?>,
            datasets: [{
                label: 'Attendance %',
                data: <?php echo json_encode($attendanceTrend->pluck('avg_attendance')->toArray()); ?>,
                borderColor: '#10b981',
    ...
    new Chart(document.getElementById('ratingAnalyticsChart'), {
        type: 'bar',
        data: {
            labels: <?php echo json_encode($driverLabels); ?>,
            datasets: [{
                label: 'Customer Rating',
                data: <?php echo json_encode($customerByDriver->values()->toArray()); ?>,
    ...
    new Chart(document.getElementById('peerAnalyticsChart'), {
        ...
        data: {
            ...
            datasets: [{
                label: 'Peer Score',
                data: <?php echo json_encode($peerByDriver->values()->toArray()); ?>,
    ...
    new Chart(document.getElementById('comparativeChart'), {
        type: 'bar',
        data: {
            labels: <?php echo json_encode($comparative->keys()->map(fn($id) => $driverLabels[$id] ?? 'Driver')->toArray()); ?>,
            datasets: [
                { label: 'Performance', data: <?php echo json_encode($comparative->pluck('performance')->toArray()); ?>, backgroundColor: '#F44336', borderRadius: 8 },
                { label: 'KPI', data: <?php echo json_encode($comparative->pluck('kpi')->toArray()); ?>, backgroundColor: '#3b82f6', borderRadius: 8 }
            ]
        },
        options: { ...chartDefaults, scales: { y: { beginAtZero: true, max: 100 } } }
    });
...
<?php echo $__env->make('admin.layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ADMIN\Herd\TEAM-3\resources\views/admin/performance/performance-analytics.blade.php ENDPATH**/ ?>